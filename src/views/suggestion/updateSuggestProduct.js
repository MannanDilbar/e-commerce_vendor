...
const UpdateProduct = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get('/api/v1/suggest-items/' + location.state.id,
                {
                    headers: {
    ...
    const submit = async () => {
        ...
        const response = await axios.put('/api/v1/suggest-items/'+id,
            formData,
            {
                headers: {
        ...
        setSpinner(false);
        MySwal.fire({
            title: <p>The Suggest Product is Updated</p>,
            timer: 2000,
            icon: 'success',
            didOpen: () => {
            },
    ...
    const getFormData = () => {
        ...
        for (let i = 0; i < images.length; i++)
            if (!typeof images[i] === 'string' &&!images[i].includes(BASE_Image_URL))
                formData.append(`images[${i}]`, images[i]);
            else if(typeof images[i] === 'string' &&images[i].includes(BASE_Image_URL))
            formData.append(`oldImages[${i}]`, images[i].replace(BASE_Image_URL, ""));
        return formData;
                ...
                <CCol md={11} lg={9} xl={7} className='mb-4' style={{ borderRadius: "50%" }}>
                    <CCard >
                        <CCardBody className='text-center'>
                            <h4 style={{ color: colors2[2] }}>Update Suggest Product</h4>
                        </CCardBody>
                    </CCard>
                </CCol>
